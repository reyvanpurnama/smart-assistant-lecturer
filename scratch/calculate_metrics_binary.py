...
def generate_visualization(x_ai, y_dosen, output_path):
    try:
        ...
        # Plot scatter points
        plt.scatter(x_ai, y_dosen, color='#6366f1', alpha=0.8, edgecolors='#4f46e5', s=70, label='Mahasiswa (N=33)')
        ...
        # Set titles & labels
        plt.title('Perbandingan Skor Otomatis AI vs Skor Manual Dosen\n(Smart Assistant Lecturer - Binary Strict Evaluation)', fontsize=12, fontweight='bold', pad=15)
        plt.xlabel('Skor Penilaian AI (Binary)', fontsize=10)
...
def main():
    csv_path = os.path.join(os.path.dirname(__file__), "../docs/IF23A_cleaned_binary.csv")
    img_output_path = os.path.join(os.path.dirname(__file__), "../docs/sebaran_nilai_binary.png")
    ...
    print("\n=== HASIL EVALUASI STATISTIK (BINARY STRICT) ===")
