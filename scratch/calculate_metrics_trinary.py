...
def generate_visualization(x_ai, y_dosen, output_path):
    try:
        ...
        # Plot scatter points
        plt.scatter(x_ai, y_dosen, color='#8b5cf6', alpha=0.8, edgecolors='#7c3aed', s=70, label='Mahasiswa (N=33)')
        ...
        # Set titles & labels
        plt.title('Perbandingan Skor Otomatis AI vs Skor Manual Dosen\n(Smart Assistant Lecturer - Trinary Evaluation)', fontsize=12, fontweight='bold', pad=15)
        plt.xlabel('Skor Penilaian AI (Trinary)', fontsize=10)
...
def main():
    csv_path = os.path.join(os.path.dirname(__file__), "../docs/IF23A_cleaned_trinary.csv")
    img_output_path = os.path.join(os.path.dirname(__file__), "../docs/sebaran_nilai_trinary.png")
    ...
    print("\n=== HASIL EVALUASI STATISTIK (TRINARY) ===")
