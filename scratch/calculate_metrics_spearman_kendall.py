...
def get_ranks(x):
    ...
    while i < n:
        ...
        while j < n and sorted_x[j][1] == sorted_x[i][1]:
            j += 1
        # Average rank for the tied values
        avg_rank = (i + 1 + j) / 2.0
        for k in range(i, j):
...
def main():
    ...
    img_output_path = os.path.join(os.path.dirname(__file__), "../docs/sebaran_nilai_spearman_kendall.png")
    ...
    print(f"1. Spearman's Rank Correlation Coefficient (rho) : {spearman_r:.4f}")
    print(f"2. Kendall's Rank Correlation Coefficient (tau)  : {kendall_t:.4f}")
    print("=========================================\n")
