...
| :--- | :--- | :--- | :--- |
| **1. Decoupling Middleware Architecture** | **Claes (2025)**<br><br>**Pack et al. (2024)** | Membangun *middleware* terisolasi berbasis *Mediator Pattern* untuk memisah autograder platform dari service LLM.<br>Uji longitudinal membuktikan LLM *closed-source* mengalami fluktuasi skor akibat pembaruan sepihak dari vendor. | Arsitektur pemisah (*decoupling*) antara Web App SAL dan Engine AI.<br><br>Penggunaan model *open-weight* (di-host mandiri via Groq API) untuk menghindari instabilitas vendor. |
| **2. Modular Chain-of-Thought (CoT) Prompting** | **Stahl et al. (2024)**<br><br>**Agarwal et al. (2025)** | Membuktikan teknik CoT (*Feedback_dCoT -> Scoring*) menghasilkan *joint scoring & feedback* yang paling transparan & akurat.<br>Model Card OpenAI **GPT-OSS-120B**: Model *open-weight reasoning* setara o3-mini via *Variable CoT* & *Harmony Format*. | AI wajib mengurai log penalaran rasional (CoT) terlebih dahulu sebelum merilis rincian nilai akhir.<br><br>Penerapan hirarki kanal `System` vs `User` untuk mencegah manipulasi prompt (*prompt injection*). |
| **3. 3-Point Partial Credit Scoring Scheme** | **Chen & Wan (2024)**<br><br>**Haller et al. (2024)**<br><br>**Mizumoto & Eguchi (2023)** | Membuktikan efektivitas rubrik 3 poin bergradasi (0, 0.5, 1.0) berbasis *multi-item rubrics* untuk keadilan penilaian.<br>Taksonomi *Automatic Short Answer Grading (ASAG)* menetapkan perlunya nilai parsial daripada biner (*all-or-nothing*).<br>Mengevaluasi kesepakatan manusia-mesin berbasis *Exact/Adjacent Agreement* pada skala skor bertingkat. | Skala nilai parsial 3 tingkat (0, 50, 100) pada rubrik penilaian jawaban praktikum SAL.<br><br>Memposisikan SAL pada domain ASAG (ketepatan semantik jawaban pendek).<br><br>Pengujian kesepakatan nilai bergradasi terhadap *ground truth* dosen. |
| **4. Knowledge Grounding Module** | **Ji et al. (2023)** | Mengklasifikasikan **Halusinasi Intrisik** (kontradiksi fakta acuan) & **Halusinasi Ekstrinsik** (fakta liar tak terverifikasi) akibat *Parametric Bias*. | Modul pengikat konteks (modul, rubrik, & *reference answer*) untuk membasmi 100% halusinasi AI penilai. |
